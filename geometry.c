...
void geo_get(GEOMETRY *geo,int device,int user_device,int all)
{
	...
	for (pass = 0; pass < md_array_info.nr_disks; ++pass) {
...
int geo_comp_addr(GEOMETRY *geo,int offset,SECTOR_ADDR *addr)
{
 ...
 /*   Always use CHS addressing on floppies:     JRC   */
	...
	addr->head = sector >> 16;
        if (linear && sector>>24)
	    die("sector %d too large for linear mode"
					" (try 'lba32' instead)", sector);
	if (verbose > 4)
	    printf("fd %d: offset %d -> dev 0x%02x, linear %d\n",
		         geo->fd, offset, addr->device, sector);
    }
    else {
